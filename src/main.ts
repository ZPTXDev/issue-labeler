...
async function run() {
    ...
    const enableVersionedRegex = parseInt(getInput("enable-versioned-regex", { required: true }));
    const versionedRegex = new RegExp(getInput("versioned-regex", { required: false }));
    ...
    const rejected = (await Promise.allSettled(promises)).map((p) => p.status === "rejected" && p.reason).filter(Boolean);
...
async function loadConfig(client: GitHubClient, configPath: string) {
    try {
        ...
        if (fs.existsSync(configPath)) {
            ...
            if (!("content" in data)) {
                throw new TypeError("The configuration path provided is not a valid file. Exiting");
            }
...
async function addLabels(client: GitHubClient, issue_number: number, labels: string[]) {
    try {
